...
onComInit comModule comInit = do protoMap <- stmRead clProtocols
                                 case ciProtocolID comInit `M.lookup` protoMap of
                                    Nothing -> logM "Client.Communitcation" "onComInit" InvalidPacket "ComInit over an unknown protocol"
                                    Just f -> do comMod <- liftSTM $ readTVar comModule
                                                 ...
                                                 if b then logM "Client.Communication" "onComInit" Normal $ "New Communication accepted on comID : " ++ show (ciComID comInit) ++ " with the protocol : " ++ show (ciProtocolID comInit)
                                                 else logM "Client.Communication" "onComInit" InvalidPacket "ComInit received for an already used comID"
