...
routingCryptoCallback cV tV pubK uK uID rV = genCryptoCallback cV tV pipeTimeOut rV inFun outFun
    where inFun :: HashFunction Request
          ...
          outFun p (RoutingAnswer onTO regM rL) = return (map runDataCB <$> regM, map (forgePacket p) rL, onTO)
          forgePacket :: Packet -> Request -> Packet
          forgePacket p r = p{introContent = IntroContent . encode $ signReq (keyID p) r}
          signReq kH r = let r' = r{roadPosition = roadPosition r + 1} in r'{neighborSignature = sign uK $ reqNeighHash kH r'}
